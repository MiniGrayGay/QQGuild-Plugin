    ...
    async init(hMsg, hReady, hInvalid, hUrl) {
        ...
        let wss = () => {
            ...
            let heartbeatU = () => {
                ...
                logger.debug("[QQ频道插件] 心跳.首次")
            }
            let heartbeatV = () => {
                ws.send(JSON.stringify({ op: 1, d: heartbeatS }))
                logger.debug("[QQ频道插件] 心跳.", heartbeatS)
            }

            ws.on("error", (error) => {
                logger.debug("[QQ频道插件] WebSocket 连接错误.", error)
            })
            ...
            ws.on("close", (code, reason) => {
                ...
                logger.debug("[QQ频道插件] WebSocket 断开连接.", code, reason.toString(), state)

                if (heartbeatO) {
                    heartbeatO = clearInterval(heartbeatO)
                    logger.debug("[QQ频道插件] WebSocket 停止心跳.")
                }

                if (!heartbeatM) {
                    heartbeatM = setInterval(wss, 3000)
                    logger.debug("[QQ频道插件] WebSocket 开始重连.")
                }
            })

            ws.on("open", () => {
                logger.debug("[QQ频道插件] WebSocket 连接成功.")
                if (heartbeatM) {
                    heartbeatM = clearInterval(heartbeatM)
                    logger.debug("[QQ频道插件] WebSocket 停止重连.")
                }
            })

            ws.on("ping", (data) => {
                logger.debug("[QQ频道插件] WebSocket Ping.", data)
            })

            ws.on("pong", (data) => {
                logger.debug("[QQ频道插件] WebSocket Pong.", data)
            })
            ...
            ws.on("message", (data, isBinary) => {
                ...
                switch (msg.op) {
                    ...
                    case 10:
                        ...
                        return
                    case 11:
                        logger.debug("[QQ频道插件] 心跳反应.", msg)
                        if (!heartbeatO) {
                            heartbeatO = setInterval(heartbeatV, heartbeatX)
                            logger.debug("[QQ频道插件] 开始心跳.")
                        }
                        return
                    default:
